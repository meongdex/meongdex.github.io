...
- **2026-07-04 (batch 28-29 DIREVERT)** — Auth & sync (akun login Google/Facebook) pernah diimplement di batch 28-29, tapi **dihapus di batch 30** karena developer memutuskan tidak mau ribet setup manual provider OAuth. Semua code auth/sync (Auth module, DriveSync, FbSync, triggerSyncAfterSave, setSyncIndicator, storage-choice screen, set-account button, sync-indicator, CONFIG.AUTH, auth fields di Store.defaults) dihapus bersih. Import/export manual tetap sebagai satu-satunya cara pindah device. Privacy policy page tetap dipertahankan (masih relevan untuk data lokal).
- **2026-07-04 (batch 30)** — Cleanup: hapus semua code auth/sync (batch 28-29) supaya codebase rapi kembali. Dihapus: CONFIG.AUTH, 7 auth fields di Store.defaults, layar storage-choice, modul Auth/DriveSync/FbSync/triggerSyncAfterSave/setSyncIndicator, tombol "Akun & Sync" di Pengaturan, sync-indicator di Beranda, 4 triggerSyncAfterSave calls di saveCat/favorit/rename/delete, updateAccountStatus(), semua CSS terkait (storage-choice, sync-indicator, home-top-right, account-info-box). Total ~700 baris dihapus dari app.js + style.css + index.html. Privacy policy page (privacy.html) tetap dipertahankan tapi diupdate untuk remove section Google/FB sync. Import/export manual tetap sebagai satu-satunya cara pindah device. Verifikasi: node --check PASS, local server HTTP 200, no auth remnants di app.js/style.css/index.html.
