...
- **2026-07-04 (batch 25)** — Bagian G1 + G2 + G3 addendum (kepribadian kucing & kedalaman naratif). (G1) Tag temperamen self-report saat menyimpan kartu: konstanta `TEMPERAMENTS` array dengan 7 pilihan (Belum diketahui, Pemalu, Manja, Waspada, Usil, Cuek, Ramah). Picker `.temp-opt` di-render di `renderColorPicker()` sebelah picker warna bulu, opsional (default "Belum diketahui"), field `c.temperament` disimpan di entri kucing. Reset `selectedTemperament = 'unknown'` di `buildNewCard` untuk cat berikutnya. Tag temperamen ditampilkan di detail kartu (`tag-row`) kalau bukan 'unknown'. Murni self-report, sama jujurnya dengan tantangan foto honor-system — tidak ada klaim AI mendeteksi kepribadian. (G2) Efek temperamen ke flavor text: `QUOTES_BY_TEMPERAMENT` map dengan 2 quote per temperamen (12 quote baru). `quoteForColor(color, temperament)` sekarang 50% chance pakai quote temperamen kalau diisi (bukan 'unknown'), fallback ke quote warna. `buildNewCard` lewat `selectedTemperament` ke `quoteForColor`. Badge achievement baru "Paham Karakter" (id `temperament10`) — terbuka saat pemain sudah mengisi temperamen untuk minimal 10 kucing berbeda, mendorong pemain benar-benar memperhatikan tiap kucing, bukan cuma spam simpan cepat. (G3) "Titipan kecil" memento naratif per kucing: di detail kartu, kalau `visits >= 3`, generate satu baris flavor kecil dari data yang SUDAH ada (galeri kunjungan + waktu-waktu kunjungan + temperamen) — contoh "[Nama] biasa muncul pas sore — terlihat manja tiap ketemu." Analisis pola waktu kunjungan (rata-rata jam → pagi/siang/sore/malam) + temperamen. Murni kosmetik, bukan currency atau field baru — konsisten dengan filosofi tanpa-monetisasi & tanpa-ekonomi-buatan proyek. CSS `.memento-block` dengan gradient blush. Verifikasi: node --check PASS, local server HTTP 200.
...
- **2026-07-04 (batch 29)** — Penyempurnaan sistem auth & sync. (1) Pull otomatis dari server saat app load (`Auth.pullFromServerBackground()`) untuk cross-device sync di awal session — kalau pemain main di HP lain tadi, progres terbaru langsung muncul di device ini. Non-blocking, UI render duluan, pull update data di background + toast "Progres diperbarui dari server" + re-render home/dex. `DriveSync.pullIfNewer()` cek apakah server lebih kaya/baru, kalau ya apply ke lokal. (2) Auto-sync setelah aksi lain: `triggerSyncAfterSave()` dipanggil di handler favorit toggle, rename cat, delete cat — bukan cuma saveCat. (3) Token refresh & error handling: `Auth.isTokenLikelyExpired()` cek Google token (>55 menit = expired, karena Google token 1 jam). `DriveSync.syncNow()` dan `pullIfNewer()` return `{ expired:true }` kalau token expired, `triggerSyncAfterSave()` tampilkan toast "Sesi login kedaluwarsa, login ulang di Pengaturan". Error 401 dari Drive API di-catch + return expired flag. (4) Sync indicator UI di Beranda: elemen `#sync-indicator` di home-top sebelah xp-pill, 4 state: idle (hidden), syncing (spinner terracotta berputar), synced (checkmark teal, fade out 2.5s), error (warning merah, persistent). Hormati `prefers-reduced-motion` (spinner tidak berputar). `setSyncIndicator(state)` function central. (5) `updateAccountStatus()` dipanggil setelah sync sukses supaya deskripsi tombol "Akun & Sync" di Pengaturan update dengan lastSyncAt terbaru. Verifikasi: node --check PASS, local server HTTP 200.
...
- `prefers-reduced-motion: reduce` global block di style.css diperluas untuk mencakup tilt 3D (E1), reveal ceremony (E2), maskot blink (E3), streak bounce (I1), parallax hero (K1) — semua di-disable kalau preferensi aktif.
- Target sentuh minimal ~44x44px untuk semua elemen interaktif baru (lightbox close button 44x44, coach-mark dismiss area = full screen, temperamen picker button, mode latihan button).
- Kontras WCAG AA 4.5:1 sudah diverifikasi untuk warna legendaris baru `#C2185B` (5.87:1 dengan putih) di batch 22.
- Estimasi ukuran unduhan model AI (TensorFlow.js COCO-SSD) sudah ditangani lewat pesan loading yang sudah ada di app.js (`#verify-loading` dengan spinner) — tidak perlu tambahan eksplisit karena model di-cache setelah unduh pertama.
- Opsi "mode kontras tinggi" dicatat sebagai ide masa depan (opsional, bukan wajib) — palet Meongdex sudah lolos WCAG AA, opsi AAA bisa ditambah kalau ada permintaan pemain.
